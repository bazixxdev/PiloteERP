...
test("une édition vit une semaine entre la direction, la RAF, le pilote, un contributeur et le responsable de pôle", async ({ page }) => {
  ...
  await panel.getByTestId("add-deliverable-label").fill("Bilan intermédiaire");
  await panel.getByTestId("add-deliverable-date").fill(deliverableDue);
  await expect(panel.getByTestId("add-deliverable-submit")).toBeEnabled();
  ...
  await expect(line).toContainText("J-10");
  await page.getByRole("tab", { name: "Budget" }).click();
