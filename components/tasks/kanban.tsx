...
export function TaskKanban({ tasks, lists, editions }: { tasks: TaskView[]; lists: (ListOpt & { editionName?: string | null })[]; editions: EditionOpt[] }) {
  ...
  const columns: { id: string | null; name: string; color: string | null; editionName?: string | null }[] = [{ id: null, name: "À trier", color: null }, ...lists.map((l) => ({ id: l.id, name: l.name, color: l.color ?? null, editionName: l.editionName }))];
  const drop = (listId: string | null, e: React.DragEvent) => {
    e.preventDefault(); setOver(null);
    const id = e.dataTransfer.getData("text/task");
    if (!id) return;
    const t = open.find((x) => x.id === id);
    if (!t || (t.listId ?? null) === listId) return;
    run(() => updateTask(id, { listId }), () => toast.success(listId ? `Rangée dans « ${columns.find((c) => c.id === listId)?.name} »` : "Remise à trier"));
  };
  return (
    <div className="flex gap-3 overflow-x-auto pb-3" data-testid="tasks-kanban">
      {columns.map((c) => {
        const items = open.filter((t) => (t.listId ?? null) === c.id).sort((a, b) => (a.dueDate ?? "9999").localeCompare(b.dueDate ?? "9999"));
        const key = c.id ?? "none";
        return (
          <section
            key={key} data-testid={`kanban-col-${key}`} data-name={c.name}
            className={cn("flex w-[272px] shrink-0 flex-col rounded-lg border bg-[#f6f8f9] transition-colors", over === key && "border-primary bg-info-soft/60")}
            ...
            onDrop={(e) => drop(c.id, e)}
          >
            <header className="flex items-center gap-2 px-3 pb-1 pt-2.5">
              {c.id ? <span className="size-2.5 shrink-0 rounded-full" style={{ background: noteColor(c.color)?.hex ?? "var(--border)" }} aria-hidden /> : <Inbox className="size-3.5 text-muted-foreground" aria-hidden />}
              ...
              {c.id && (
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-48">
                    <DropdownMenuItem asChild><Link href={`/taches?liste=${c.id}`}>Ouvrir la liste (réglages, partage)</Link></DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
              )}
            </header>
            ...
            <AddInColumn listId={c.id} pending={pending} run={run} />
          </section>
        );
      })}
    </div>
...
function AddInColumn({ listId, pending, run }: { listId: string | null; pending: boolean; run: Run }) {
  ...
  const submit = () => {
    ...
    run(() => addTask({ label: clean, dueDate: bangDue, listId }), () => setLabel(""));
  };
  return (
