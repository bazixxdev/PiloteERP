...
export function PerimeterChips({ current, poleName, hrefFor }: { current: Perimeter; poleName: string | null; hrefFor: (p: Perimeter) => string }) {
  ...
  const chip = (p: Perimeter, label: string) => (
    <Link key={p} href={hrefFor(p)} data-testid={`perimeter-${p}`} aria-current={current === p ? "page" : undefined} className={cn("h-8 rounded-full border px-3 text-sm leading-8 transition-colors", current === p ? "border-primary bg-primary text-white" : "bg-card hover:bg-muted")}>{label}</Link>
  );
  return (
