    ...
    <div className="mb-5 flex flex-wrap items-center justify-between gap-x-6 gap-y-3">
      <div className="flex min-w-0 flex-[1_1_18rem] items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          {/* Sous-titre borné à une largeur de lecture : il ne court pas jusqu'aux actions (retour du 17/09). */}
          {subtitle && <p className="mt-1 max-w-[64ch] text-xs text-muted-foreground">{subtitle}</p>}
          {children}
        </div>
      </div>
      {actions && <div className="ml-auto flex flex-wrap items-center gap-2 self-center md:justify-end">{actions}</div>}
    </div>
  );
}
