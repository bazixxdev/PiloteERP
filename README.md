...
5. `npm run lint && npm run build` — vérifications ; `npm test` — 29 tests Playwright sur leur propre serveur (port 3100, `.next-test`). **Ils reseedent `prisma/prototype.db`** : ne jamais lancer deux suites en même temps (même base, même serveur, même `test-results`).
