...
- **CRITICAL** — on déploie **uniquement** par `./deploy/deploy.sh <cress|tlst>` depuis un arbre git propre sur `main` : le script lance d'abord `npm run check:full` en local (journal dans `.deploy-logs/check-full-<instance>-<stamp>.log`, les premières erreurs s'affichent) et s'arrête si une suite est rouge — pas de variable pour sauter cette étape ; puis build avant bascule, dump `pg_dump -Fc` validé par `pg_restore --list` + archive médias (mode 0600, `/var/backups/<instance>/`), migration, bascule, healthcheck, `.release.json` (commit déployé, release précédente). Jamais de `rsync`, `git pull` ou `prisma migrate` à la main sur le serveur.
- **NEVER** modifier le `.env` d'une instance à la main sans rejouer `deploy.sh` : `BETTER_AUTH_URL` doit valoir exactement `<PUBLIC_URL>/api/auth` (sous-chemin compris), sinon la connexion répond 404 — `deploy.sh` le vérifie et sonde la route de connexion après démarrage (cassé du 20 au 26/09 par une réécriture manuelle).
