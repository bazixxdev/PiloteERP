...
export default async function AnnuelPage({ searchParams }: { searchParams: Promise<{ annee?: string; pole?: string }> }) {
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      {sp.pole && (
        <Section title={`${cap(pl(V.projet))} ${du(V.pole)}`} description={`Pour la réunion ${de(V.pole)} : statut et alertes de chaque ${V.projet.one}.`} className="mb-4">
          {poleEditions.length === 0 ? <p className="text-sm text-muted-foreground">{`${cap(aucun(V.projet))} cette année.`}</p> : (
              ...
              return (
                <tr key={p.id} className="align-top hover:bg-muted/30">
                  <td className="sticky left-0 z-10 bg-card px-3 py-2">
                    ...
                    <div className="text-muted-foreground">{p.pole?.name ?? "transversal"} · {myEditions.length}{` ${myEditions.length > 1 ? pl(V.projet) : V.projet.one}`}</div>
      ...
      <p className="mt-3 text-xs text-muted-foreground">{`Chaque pastille est un jalon d'${V.action.one} dont la personne est responsable ; les jours prévus (charge) se règlent sur ${le(V.edition)} de chaque ${V.projet.one}, onglet Temps, ou au séminaire.`}</p>
    </div>
  );
}
