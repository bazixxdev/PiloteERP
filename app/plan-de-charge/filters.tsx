...
export function LoadFilters({ poles, current, allValue }: { poles: Opt[]; current: { debut: string; horizon: string; pole: string; vue: string; avenir: string }; allValue: string }) {
  ...
  return (
    <div className="mb-3 flex flex-wrap items-center gap-2" data-testid="load-filters">
      ...
      <Filter label={cap(pl(V.edition))}><Select className={sel} value={current.avenir} onChange={(e) => set("avenir", e.target.value)} aria-label={`${cap(pl(V.edition))} incluses`}><option value="">En cours et à venir</option><option value="non">En cours seulement</option><option value="seul">À venir seulement</option></Select></Filter>
