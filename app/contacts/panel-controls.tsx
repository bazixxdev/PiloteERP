...
export function DeleteContactButton({ id, own, cited }: { id: string; own: boolean; cited: boolean }) {
  const [pending, start] = useTransition();
  const router = useRouter();
  ...
  return <div><Button size="sm" variant="ghost" disabled={pending} className="text-danger hover:text-danger" onClick={() => { if (confirm("Supprimer ce contact ? Il disparaît des listes qui le contiennent.")) start(async () => { const r = await deleteContact(id); if (!r.ok) { toast.error(r.error); return; } toast.success("Contact supprimé"); router.push("/contacts"); router.refresh(); }); }} data-testid="contact-delete"><Trash2 />Supprimer{own ? "" : " (admin)"}</Button></div>;
}
