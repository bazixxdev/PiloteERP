...
export default async function CodirPage({ searchParams }: { searchParams: Promise<{ pole?: string; plein?: string }> }) {
  ...
  const Block = ({ icon: Icon, title, count, tone, children }: { icon: typeof Flag; title: string; count: number; tone: "danger" | "warning" | "coral"; children: React.ReactNode }) => (
    <section className={cn("rounded-2xl border bg-card p-5", count === 0 && "opacity-60")} data-testid={`codir-${title.toLowerCase().replace(/[^a-z]+/g, "-")}`}>
      <h2 className={cn("mb-3 flex items-center gap-2 font-semibold", big ? "text-2xl" : "text-base")}>
        <Icon className={cn("size-5", tone === "danger" ? "text-danger" : tone === "warning" ? "text-warning" : "text-coral")} />{title}
        <span className={cn("ml-1 rounded-full px-2 text-sm font-semibold", count ? (tone === "danger" ? "bg-danger-soft text-danger" : "bg-warning-soft text-[#8a5a00]") : "bg-muted text-muted-foreground")}>{count}</span>
      </h2>
      {count === 0 ? <p className="text-sm text-muted-foreground">Rien à traiter.</p> : children}
    </section>
  );
  ...
  return (
    <div className={cn("p-6", big && "text-lg")}>
      ...
      <PageHeader
        title="Écran CODIR"
        subtitle={<span>{total} point{total > 1 ? "s" : ""} à traiter sur {editions.length} éditions en cours · seulement ce qui appelle une décision · <SessionTimer minutes={20} /></span>}
        actions={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button asChild variant={big ? "outline" : "default"} className={big ? "" : "bg-coral text-white hover:bg-coral/90"}><Link href={big ? qs({ plein: "" }) : qs({ plein: "1" })}>{big ? "Quitter la projection" : "Projeter"}</Link></Button>
            <Button asChild variant="outline"><Link href="/portefeuille">Portefeuille complet</Link></Button>
          </div>
        }
      />
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <div className="grid content-start gap-4">
          ...
          <Block icon={Clock} title="Livrables financeurs proches" count={deliverables.length} tone="warning">
            <ul>{deliverables.map(({ e, f, d, days }) => <Line key={d.id} e={e} decisionLabel={`Livrable « ${d.label} » (${f.funder.name})`}>{d.label} <span className="text-muted-foreground">· {f.funder.name} · {fmtDate(d.dueDate)}</span> <span className={days < 0 ? "text-danger" : "text-[#8a5a00]"}>· {days < 0 ? `${-days} j de retard` : `J-${days}`}</span></Line>)}</ul>
          </Block>
          <Block icon={Euro} title="Enveloppes en alerte" count={envelopes.length} tone="warning">
            <ul>{envelopes.map((e) => <Line key={e.id} e={e} decisionLabel="Enveloppe">{fmtEuro(e.used)} consommés sur {fmtEuro(e.budgetEnvelope)} <span className={cn(e.used > e.budgetEnvelope! ? "text-danger" : "text-[#8a5a00]")}>· {Math.round((e.used / e.budgetEnvelope!) * 100)} %</span> <span className="text-muted-foreground">· disponible {fmtEuro(e.remaining)}</span></Line>)}</ul>
          </Block>
          <Block icon={AlertTriangle} title="Temps hors objectif" count={timeOver.length} tone="warning">
            <ul>{timeOver.map(({ e, a, consumed }) => <Line key={a.id} e={e} decisionLabel={`Temps « ${a.name} »`}>{a.name} <span className="text-[#8a5a00]">· {fmtNumber(consumed, 0)} h sur {a.timeTarget} h</span> <span className="text-muted-foreground">· {a.owner?.name ?? "—"}</span></Line>)}</ul>
          </Block>
        </div>
      </div>
          ...
          <ul className="divide-y text-sm" data-testid="recent-decisions">
            {recent.map((d) => (
              <li key={d.id} className="py-1.5">
                <span className="rounded-full bg-secondary px-1.5 text-[11px] font-medium text-primary">{refs.decision_instance?.[d.instance]?.label ?? d.instance}</span> <Link href={`/edition/${d.editionId}?onglet=validations`} className="font-medium hover:underline">{d.edition.project.name} · {d.edition.year}</Link> — {d.body}
