        ...
        <div className="scroll-shadow-x overflow-auto rounded-md border" tabIndex={0} aria-label={`Tableau des ${rows.length} ${pl(V.edition)}, défilement horizontal et vertical`}>
          ...
          <table className="w-full min-w-[760px] text-[13px] xl:min-w-[900px]" data-testid="portfolio-table">
            <thead className="sticky top-0 z-[2] bg-[#f1f5f6] text-left text-[10px] font-semibold text-muted-foreground">
              <tr>
                <th className="px-3 py-2.5 whitespace-nowrap">{`Projet / ${V.edition.one} · alertes`}</th>
                <th className="px-3 py-2.5 whitespace-nowrap">{`${cap(V.pole)} · ${V.pilote.one}`}</th>
                ...
                return (
                  <tr key={r.id} className="border-t border-[#e3e9eb] align-middle hover:bg-[#f8f9f3]" data-alert={r.hasDanger ? "danger" : r.alerts.length ? "warning" : "none"}>
                    <td className={cn("min-w-[220px] max-w-[280px] px-3 py-3", r.hasDanger ? "border-l-[3px] border-l-danger" : r.alerts.length > 0 ? "border-l-[3px] border-l-warning" : "border-l-[3px] border-l-transparent")}>
                      ...
                      <small className="mt-1 block text-[10px] text-muted-foreground">{`${cap(V.edition)} `}{r.year} · {r.project.analyticCode}</small>
