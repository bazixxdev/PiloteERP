...
export async function GET(req: Request) {
  ...
  const loaders: Record<string, () => Promise<Record<string, unknown>[]>> = {
    ...
    editions: async () => (await prisma.edition.findMany({ include: { project: true } })).map((e) => ({ projet: e.project.name, annee: e.year, statut: e.status, decision: e.decisionDate, enveloppe: e.budgetEnvelope, engage: e.committed, realise: e.spent, enjeux: e.stakes, objectifs: e.operationalObjectives, bilan: e.report })),
  ...
  if (table === "tout" || format === "json") {
    ...
    return new NextResponse(JSON.stringify(all, null, 2), { headers: { "Content-Type": "application/json; charset=utf-8", "Content-Disposition": `attachment; filename="pilote-export.json"` } });
  }
