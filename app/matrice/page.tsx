...
export default async function MatricePage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      {panel && (
        <UrlPanel title={fundingPanelTitle(panel.e, panel.f)} description={<>{canEditFunding(me) ? FUNDING_PANEL_DESCRIPTION.rw : FUNDING_PANEL_DESCRIPTION.ro} · <Link href={`/edition/${panel.e.id}?onglet=budget#recettes`} className="text-primary hover:underline">{`ouvrir ${le(V.edition)}`}</Link></>} closeHref={qs({})} testId="matrix-panel" wide>
