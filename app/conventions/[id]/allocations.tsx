...
export function DetachButton({ lineId, editionLabel }: { lineId: string; editionLabel: string }) {
  ...
  return (
    <Button size="xs" variant="ghost" disabled={pending} title={`Détacher ${editionLabel} de cette convention`} data-testid={`detach-${lineId}`}
      onClick={() => { if (!confirm(`Détacher « ${editionLabel} » de cette convention ?\nLa ligne de financement redevient annuelle, propre à l'édition ; vide, elle est supprimée.`)) return; run(() => detachFundingLineFromConvention(lineId), (r) => toast.success(r.ok && r.data?.deleted ? "Ligne vide supprimée" : "Édition détachée : sa ligne redevient un financement annuel")); }}>
      <Unlink />Détacher
    </Button>
  );
}
