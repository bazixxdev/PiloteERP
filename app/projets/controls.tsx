  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <form className="relative" onSubmit={(e) => { e.preventDefault(); go({ q: ((new FormData(e.currentTarget).get("q") as string) ?? "").trim() }); }}>
        ...
        <Input name="q" defaultValue={q} placeholder="Projet, code, pôle, pilote…" className="h-8 w-56 pl-8 text-xs" aria-label="Rechercher un projet" data-testid="projects-search" />
      </form>
