    ...
    return (
      <span className="inline-flex items-center gap-1 rounded-[5px] border bg-card px-2 py-1 text-[11px]" data-testid="edition-years">
        {[...sorted].reverse().map((x) => (
          <Link key={x.id} href={`/edition/${x.id}`} aria-current={x.id === currentId ? "page" : undefined} className={cn("rounded-sm px-1.5 py-0.5", x.id === currentId ? "bg-primary font-semibold text-white" : "text-muted-foreground hover:bg-muted")}>Édition {x.year}</Link>
        ))}
      </span>
    );
  }
