  ...
  return (
    <div className="grid gap-1 text-[11px]" data-testid={`invoice-${id}`}>
      <div className="flex flex-wrap items-center gap-1">
        ...
        {canTrack && !receivedAt && <Button size="xs" variant="outline" disabled={pending} onClick={() => run(() => markInvoice(id, "received"), "Facture reçue : le pilote est prévenu")} data-testid={`invoice-received-${id}`}>Reçue</Button>}
        {canTrack && receivedAt && !paidAt && <Button size="xs" className="bg-mint text-white hover:bg-mint/90" disabled={pending} onClick={() => run(() => markInvoice(id, "paid"), "Facture payée : le pilote est prévenu")} data-testid={`invoice-paid-${id}`}>Payée</Button>}
