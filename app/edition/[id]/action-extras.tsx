  ...
  return (
    <div className="mt-0.5 flex items-center gap-1 text-[10px] text-muted-foreground">
      <RowPanel testId={`${V.action.one}-extras-${index}`} openTestId={`${V.action.one}-details-${index}`} title={name} description={`Contenu, lieu, participants, ligne de financement, tâches, réalisations et temps de ${ce(V.action)}.`} label="détail" className="px-1 py-0 text-[10px]" hint="Contenu, lieu, participants, ligne de financement, public" wide>
        {children}
        {canDuplicate && (
          <button type="button" disabled={pending} onClick={() => start(async () => { const r = await duplicateAction(actionId); if (!r.ok) toast.error(r.error); else { toast.success(`${cap(V.action)} dupliquée : renommez-la et posez son jalon`); router.refresh(); } })} className="inline-flex w-fit items-center gap-1 rounded-sm px-1 text-[11px] text-muted-foreground hover:bg-muted hover:text-primary" data-testid={`${V.action.one}-duplicate-${index}`} title="Dupliquer (même contenu, lieu, participants ; jalon à poser)">
            <Copy className="size-3" />{`Dupliquer ${ce(V.action)}`}</button>
        )}
      </RowPanel>
