    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-80" align="end">
        <form className="grid gap-2" onSubmit={(e) => { e.preventDefault(); start(async () => { const r = await addTask({ label, dueDate: due || null, editionId, actionId: actionId || null }); if (!r.ok) { toast.error(r.error); return; } toast.success("Tâche ajoutée à « Ma semaine »"); setOpen(false); setLabel(""); setDue(""); router.refresh(); }); }}>
          ...
          <p className="text-[11px] text-muted-foreground">Privée : elle n'apparaît que dans votre « Ma semaine », rattachée à cette édition.</p>
          ...
          <Select className="h-8 w-full rounded-lg border bg-card px-2 text-sm" value={actionId} onChange={(e) => setActionId(e.target.value)} aria-label="Action concernée"><option value="">— l'édition entière —</option>{actions.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}</Select>
