...
function RuleForm({ initial, onSubmit, pending, submitLabel }: { initial: CashRuleForm; onSubmit: (f: CashRuleForm) => void; pending: boolean; submitLabel: string }) {
  const [f, setF] = useState<CashRuleForm>(initial);
  const cats = f.direction === "in" ? CASH_CATEGORIES.in : CASH_CATEGORIES.out;
  return (
    <form className="grid gap-3 text-xs" onSubmit={(e) => { e.preventDefault(); onSubmit(f); }}>
      ...
      <div className="flex justify-end"><Button type="submit" size="sm" disabled={pending || !f.label.trim() || !f.startMonth || !String(f.amount).trim()} data-testid="rule-submit">{submitLabel}</Button></div>
    </form>
  );
}
...
export function NewRuleDialog({ defaultMonth }: { defaultMonth: string }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild><Button size="sm" data-testid="new-rule"><Plus />Nouvelle règle</Button></DialogTrigger>
      <DialogContent className="max-w-lg">
        <DialogHeader><DialogTitle>Nouvelle règle de flux</DialogTitle></DialogHeader>
    ...
    <span className="inline-flex items-center gap-1">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="max-w-lg">
          ...
          {open && <RuleForm initial={{ label: rule.label, direction: rule.direction, category: rule.category, amount: rule.amount, period: rule.period, startMonth: rule.startMonth, endMonth: rule.endMonth ?? "", notes: rule.notes ?? "" }} pending={pending} submitLabel="Enregistrer" onSubmit={(f) => run(() => updateCashRule(rule.id, f), () => { setOpen(false); toast.success("Règle modifiée"); })} />}
        </DialogContent>
      </Dialog>
