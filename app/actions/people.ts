...
export async function prepareDeparture(input: DepartureInput): Promise<Result<{ moved: number }>> {
  ...
  await Promise.all([notify(input.pilotTo, "les projets pilotés"), notify(input.guarantorTo, "les garanties de projet"), notify(input.poleLeadTo, `la responsabilité ${de(V.pole)}`), notify(input.sponsorTo, `le parrainage des ${pl(V.projet)}`), notify(input.actionsTo, `les ${pl(V.action)} en cours`), notify(input.requestsTo, "les demandes ouvertes")]);
