...
| **Postgres hébergé UE** | ENF-3, ENF-7 | **Fait le 17/09** : PostgreSQL partout (dev, tests, VPS bazixx à Paris) ; migration initiale unique ; `pg_dump` à chaque déploiement ; base de test séparée | Chez un hébergeur UE : même `DATABASE_URL`, rien d'autre. | 0 j (fait) + hébergement |
