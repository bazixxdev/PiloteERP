...
| **Teams** | EF-J2 (S), ENF-8 (V2) | Liens libres vers un canal (`DocLink`) ; fil de discussion par édition (`Comment`) | Soit se contenter du lien vers le canal (déjà fait), soit poster chaque commentaire dans le canal via un webhook entrant Teams (10 lignes dans `addComment`). | 0,5 à 1 j |
