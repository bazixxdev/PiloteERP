...
|---|---|---|
| **Contacts et listes** — **FAIT (18/09)** | livré | `Contact` (ex-`OrganisationContact`, organisation facultative, tags, adresse, `brevoContactId` / `brevoStatus` prêts), `ContactList` (visibilité, projet, couleur, **colonnes propres** : case, texte, date, liste de valeurs), `ContactListItem` (rôle, valeurs). Écran `/contacts` : annuaire commun (recherche, mots-clés, fiche en panneau) + listes à soi / partagées (en lecture) ; import csv / xlsx en deux temps (en-tête trouvé sous les titres, correspondance proposée, contacts connus complétés), export CSV. |
| **Brevo** — **FAIT (18/09)** | livré, connecteur (clé dans l'environnement) | `lib/brevo.ts` (client v3, reprise sur 429) + `lib/brevo-sync.ts`. **Sens Brevo → outil** : tous les contacts du compte et leurs attributs entrent dans l'annuaire (colonnes communes reconnues PRENOM / NOM / SMS / SOCIETE / VILLE…, le reste gardé en `brevoAttributes`), rapprochement par identifiant puis e-mail, contact connu complété jamais écrasé ; désinscrits et disparus **marqués** (`brevoStatus`), jamais supprimés. **Listes Brevo suivies** (Admin › Import / export › Brevo) = miroir `ContactList.source = brevo` visible de tous, membres tenus par la synchro, attributs en colonnes (`brevo: true`, en lecture). **Sens outil → Brevo** liste par liste : « Envoyer vers Brevo » crée la liste (dossier « Pilote »), crée / complète les contacts avec e-mail (attributs que le compte connaît seulement), retire de la liste Brevo ceux retirés ici ; sans e-mail ignorés, désinscrits jamais renvoyés. Pas de synchro planifiée (bouton ; un cron viendra si besoin). Tests contre un faux Brevo (`tests/brevo-mock.mjs`). |
| **Adhérents + HelloAsso** — **FAIT (18/09)** | livré, module `adherents` (INSTANCE_MODULES) + connecteur | Table `Membership` (organisation *ou* contact × année ; collège, cotisation, statut à régler / réglée / exonérée / annulée, réglée le, moyen, `helloAssoItemId`), genre d'organisation `member`, droit `members.manage` (direction, RAF), référentiel `college`. Rubrique « Adhérents » (page `/adherents` : année, tuiles, table éditable en place, règlement en un clic, filtres, export CSV ; vue Cotisations : par collège, reste à encaisser ; reconduction N → N+1 « à régler »). Adhésions sur la fiche organisation et la fiche contact. Listes de base « Interlocuteurs · adhérents » et « Adhérents à jour · année ». Connecteur HelloAsso (`lib/helloasso.ts`, OAuth client_credentials ; `lib/helloasso-sync.ts`) : commandes → adhésions (structure = organisation créée au besoin, référent = payeur ; sans structure = personne), **une adhésion préparée à la main (reconduction) est réglée par HelloAsso, pas doublée** ; formulaires d'événement suivis → listes miroir des inscrits (tarif en rôle, champs personnalisés en colonnes). Admin › Import / export › HelloAsso. Tests contre `tests/helloasso-mock.mjs`. |
| **Trésorerie** — **FAIT (18/09)** | livré, module `tresorerie` (INSTANCE_MODULES), droits `treasury.view` (direction, RAF, responsables de pôle) / `treasury.manage` (direction, RAF) | Plan mensuel sur douze mois glissants (`lib/treasury.ts`, projection pure). **Rien n'est ressaisi** : versements attendus des financeurs (`Payment` non reçus, à la date attendue ; en retard = premier mois), factures reçues non payées et engagements à facturer (`Expense` ouvertes, premier mois par prudence), cotisations à régler (module Adhérents) ; plus les **règles de flux** saisies (`CashRule` : sens, catégorie, montant, périodicité mois / trimestre / semestre / année / une fois, premier et dernier mois, actif). Solde de départ (relevé bancaire à un mois) et seuil dans `Settings`. Page `/tresorerie` : tuiles (départ, point bas, encaissements, décaissements), courbe du solde (recharts) avec le seuil, tableau lignes × mois avec soldes colorés sous le seuil / négatifs, règles (créer, modifier, désactiver, supprimer), détail des flux dérivés avec liens, export CSV. Plus simple que le module d'erp-tlst (pas de scénarios, pas de rapprochement bancaire, pas d'IA) : à étendre si l'usage le demande. |
| **Matériel & prêts** — **FAIT (18/09)** | livré, module `materiel` (INSTANCE_MODULES), droit `equipment.manage` (direction, RAF, assistant·e) ; tout le monde emprunte et rend | `Equipment` (nom, catégorie libre avec suggestions, référence, rangement, quantité, état en état / fatigué / en panne / sorti de l'inventaire, valeur, notes) et `Loan` (matériel × quantité, à une personne de l'équipe, un contact ou une organisation de l'annuaire, pour une édition, sorti le, retour attendu, rendu le, état au retour). Disponible = quantité − sorti. Rubrique « Matériel » : Inventaire (recherche, catégorie, disponible seulement, « Prêter » par ligne) et Prêts en cours (retards en rouge, « Rendu » avec note) ; fiche en panneau (`?materiel=`) modifiable en place, historique des prêts, sortie de l'inventaire (jamais supprimé). Pas de réservation à l'avance ni de rappel automatique du retour (consigné). |
...
| Entra ID | plus tard |
| Multi-instance (lot I) | **FAIT le 18/09** (spec `docs/superpowers/specs/2026-09-18-lot-i-multi-instance-design.md`, plan `docs/superpowers/plans/…`) : `config/clients/<client>.ts` (nom court / long avec genre, logos couleur / blanc / marque / favicon dans `public/clients/<client>/`, thème complet, polices, modules, réglages, **vocabulaire** : projet, édition, action, pôle, CODIR, RAF, direction, pilote), choisi par `NEXT_PUBLIC_CLIENT` au build ; `lib/vocab.ts` (V + helpers d'article et d'accord) sur **tous** les écrans, `npm run check:vocab` (AST) = 0 mot en dur ; `lib/branding.ts` = point d'entrée unique (layout, `<Logo>`, `app/icon.tsx`) ; seeds `prisma/seeds/{common,cress,tlst}.ts` (CRESS : empreinte identique ; `npm run seed:tlst`, `--skeleton`) ; `./deploy/deploy.sh <instance>` + `deploy/instances/*.env` + `pilote@.service` + conf nginx TLST prête ; projet Playwright `tlst` (`tests/habillage.spec.ts`, 26 tests). Reste : instance TLST réelle (DNS, certificat, `deploy.sh tlst --seed`) puis reprise des données mca-vps ; vrai logo TLST à déposer dans `public/clients/tlst/` puis `python3 scripts/make-brand-images.py` ; palette et mots TLST à valider avec eux (`trésorier` au masculin faute d'un épicène lisible avec article). |
| Instance TLST | **en ligne le 18/09** : https://tlst.bazixx.fr/outilcli/tlst/pilote (rideau, fiche `CRESS/tlst-acces-demo.txt`), seed démo. **Aucune donnée réelle dans erp-tlst** (prod et préprod mca-vps : référentiels du seed + compte admin) → pas de reprise ; les 3 axes réels sont dans le seed. |
| Archivage d'`erp-tlst` (arrêt pm2 prod/préprod sur mca-vps, dépôt figé) | sur go de Gaël |
...
| **Heures supplémentaires (module RH)** — demandé par TLST le 25/09 | **feuille de route, important, pas maintenant** (Gaël). Aujourd'hui : un classeur Excel dédié au tiers-lieu (« SUIVI HORAIRE »), pas Mysilae ; les **congés**, eux, restent sur Mysilae (double validation coordination / administratif). | Ce que fait le classeur : base hebdomadaire par personne (= `Rhythm` + `PersonRhythmPeriod`, déjà là), pointage jour par jour (début, fin, pause), écart à la base, majoration 25 % sur les 8 premières heures puis 50 %, récupérations posées en heures, report du reliquat N-1, récapitulatif mensuel pour la paie, règles proposées au CA le 25/09 (plafond de 35 h cumulées, récupération posée au-delà de 3 h d'absence, pas d'échange de journées inégales un jour férié ou de congé). **Prérequis** : décider que le temps saisi devient du temps de **présence** (aujourd'hui `/temps` est une clé de répartition, « aucun solde » par choix) ; jours fériés dans l'attendu (manque aussi à la CRESS). Faisable sans nouvelle table lourde : calcul sur `TimeEntry` + `Rhythm`, solde de départ par personne et par année, taux de majoration dans les réglages ; module activable par instance. |
